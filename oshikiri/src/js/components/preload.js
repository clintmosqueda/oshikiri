...
export default function Preload() {
  ...
  if (!!preloadDiv) {
    ...
    const baseUrl =
      ...
      '/wp-content/themes/oshikiri/assets/images/';
    ...
    const preloadDivHeight = !!preloadDiv ? preloadDiv.clientHeight : 0;
    ...
    if (!!mvOrbText) {
      mvOrbText.innerHTML = !!swiperSlideText[0]
        ? swiperSlideText[0].dataset.text
        : '';
    }
    ...
    function fadeInElement() {
      ...
      timeline.fromTo(
        preloadDiv,
        { height: `${preloadDivHeight}px` },
        { height: preloadImageHeight, ease: 'expo.inOut', duration: 1.2 }
      );
      timeline.fromTo(
        preloadDiv,
        { autoAlpha: 1, display: 'block' },
        { autoAlpha: 0, display: 'none', ease: 'power1.out' }
      );
      timeline.fromTo(
        sloganHeading,
        { autoAlpha: 0, y: 100 },
      ...
      timeline.fromTo(
        header,
        { autoAlpha: 0, y: -100 },
        { autoAlpha: 1, y: 0, ease: 'power3.out', delay: 0.3 },
        '<'
      );
      timeline.fromTo(
        sloganDesc,
      ...
      timeline.fromTo(
        circleSlogan,
        { autoAlpha: 0, x: -100 },
        { autoAlpha: 1, x: 0, ease: 'power3.out', delay: 0.3 },
        '<'
      );
      timeline.fromTo(
        mvCircles,
        { autoAlpha: 0 },
        { autoAlpha: 1, ease: 'power3.out', delay: 0.5, stagger: { each: 1 } },
        '<'
      );
    ...
    function onComplete() {
      ...
      loaderTimeline.add(fadeInElement());
    }
